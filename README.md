...
- логирует сообщения из групповых чатов в PostgreSQL,
...
- **RAG-слой (Qdrant + BM25 + эмбеддинги)**
  ...
  - Возможности: поиск сообщений с фильтрами (например, по времени и отправителю), возможность очистки базы данных по аналогичным фильтрам
  - При запросе ищутся топ-k лучших элементов в каждом из представлений и переранжируются с помощью `RRF`
  - Две отдельные базы данных: одна хранит эмбеддинги из `SentenceTransformer`, другая - из `BM25Okapi` (`rank-bm25`)
  - `BM25` добавляет новые элементы сразу и считает статистики на основе уже существующего корпуса текстов. Раз в 10 минут весь корпус текста пересчитывается, чтобы уточнить статистики. Это позволяет не пересчитывать статистики по всему корпусу при каждом новом сообщении.
  - Для обработки слов использован `SnowballStemmer (ru)`
  - Векторное хранилище: `Qdrant` (docker‑сервис). _Выбирали между `Chroma` и `Qdrant`. Под наши требования больше подошел второй._
...
2. При запросе `/ask_question <текст>` или `/ask <текст>`:
   ...
   - возвращает ответ и идентификаторы использованных сообщений.

---

## Быстрый старт

1. Скопировать `.env.example` в `.env` и заполните параметры (Telegram, Postgres, Mistral):
...
- `/show #<id>` — показать исходные сообщения, использованные при ответе с id `#<id>`.

---

## Проверка наличия данных во всех БД

Можно использовать скрипт `tools/check_all_dbs.py` для проверки:
