...
- **RAG-слой (Qdrant + BM25 + эмбеддинги)**
  ...
  - `BM25` добавляет новые элементы сразу и считает статистики на основе уже существующего корпуса текстов. Раз в 10 минут весь корпус текста пересчитывается, чтобы уточнить статистики. _Это помогает не пересчитывать статистики по всему корпусу при каждом новом сообщении._
